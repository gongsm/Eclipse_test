...
class AfdxMessage(object):
    ...
    def __init__(self, name, id, row, cidx, direction):
        ...
        try:
            ...
            raise Exception, "Bad port type (SAMPLING or QUEUING expected)"

        if self.portType[0:3] == "SAM":
            self.portType = "SAMPLING"
        elif self.portType[0:3] == "QUE":
            self.portType = "QUEUING"
        else:
            raise Exception, "Bad port type (SAMPLING or QUEUING expected)"
        ...
        if self.portType[0] == "Q":
            ...
            if self.queueLength < 1:
                raise Exception, "Bad queue length for QUEUING port"
        else:
            self.queueLength    = 0
        ...
        try:
            ...
            if direction == 'output':
                raise Exception, "Bad Transmit Refresh Period"
            else:
                self.msgRate        = 0

        try:
            self.msgRxRate        = ival(row, cidx["RxRate"])        
        except:
            if direction == 'input':
...
DATATYPES_COMPAT = {
    ...
    'INT':       set(('INT', 'UINT', 'COD', 'FLOAT', 'BNR', 'UBNR', 'BCD', 'UBCD')),
    'COUNT':     set(('INT',)),
...
class InputSignal(object):
    def __init__(self, row, cidx, msgref, ddmap, paramxref, noparams=False, rowidx=0):
        ...
        self.ssel0rder        =ival(row, cidx["SselOrder"])
        if not self.ssel0rder :
            self.ssel0rder = 1
        if self.sigType == "BNR":
        ...
        if self.alertId is not None:
            ...
            self.sigDsOffset   = (MAX_ALERT_ID - self.alertId) * 2         # bit offset within DataSet
            self.sigSize       = 2;
            self.sigType       = "COD";
            ...
            if sig.paramOffset != -1 or noparams:
                if message.msgclass == 'AFDX':
                    ...
                    if not sig.paramName in dso:
                        dso[sig.paramName] = {}
                    dso[sig.paramName][sig.ssel0rder] = sig
...
def testComputeSignalAccess():
    tc = (
         ...
         (1, ("AFDX",   "INT",  16,         0,          8,      0), (6, 0, 2)),
         (2, ('AFDX',   'COD',  16,        40,          8,      0), (0, 8, 4)),
         ...
         (16,("AFDX",   "INT",   8,         7,           8,     0), "Exception"),
         (17,("AFDX",   "INT",   8,         8,           8,     0), (6, 0, 1)  ),
         (18,("AFDX",   "INT",  16,         8,           8,     0), "Exception"),
         (19,("AFDX",   "INT",  16,        16,           8,     0), (4, 0, 2)  ),
         (20,("AFDX",   "INT",  16,        24,           8,     0), "Exception"),
         (21,("AFDX",   "INT",  16,        32,           8,     0), (2, 0, 2)  ),
         (22,("AFDX",   "INT",  16,        48,           8,     0), (0, 0, 2)  ),
         (23,("AFDX",   "INT",  24,        48,           8,     0), "Exception"),
         (24,("AFDX",   "INT",  32,        48,           8,     0), "Exception"),
         (25,("AFDX",   "INT",  32,        32,           8,     0), (0, 0, 4)),
         (26,("AFDX",   "INT",  32,        0,            8,     0), (4, 0, 4)),
         (27,("AFDX",   "COD",  5,         4,            8,     0), (4, 4, 4)),
         (28,("AFDX",   "COD",  5,         12,           8,     0), (4, 12, 4)),
         (29,("AFDX",   "COD",  5,         18,           8,     0), (4, 18, 4)),
         (30,("AFDX",   "COD",  5,         30,           8,     0), (0, 30, 8)),
         (31,("AFDX",   "COD",  5,         33,           8,     0), (0, 1, 4)),
         (32,("AFDX",   "FLOAT",16,        0,            4,     0), "Exception"),
         (33,("AFDX",   "FLOAT",32,        16,           8,     0), "Exception"),
         (34,("AFDX",   "FLOAT",32,        0,            8,     0), (4, 0, 4)),
         (35,("AFDX",   "INT",  24,        0,            8,     0), "Exception"),
         (36,("AFDX",   "BYTES",24,        0,            8,     0), (5, 0, 1)),
         (37,("AFDX",   "COD", 7,        424,           148,   8), "Exception"),
         (38,("AFDX",   "COD", 7,        431,           148,   8), "Exception"),
    )
