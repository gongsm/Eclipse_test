...
class AfdxMessage(object):
    ...
    def __init__(self, name, id, row, cidx, direction):
        ...
        try:
            self.portType       = sval(row, cidx["PortType"]).upper()
        except:
        ...
        try:
            self.msgLength  = ival(row, cidx["A664MsgMaxLength"])
        except:
            raise Exception, "Bad message length value"

        try:
            self.msgRxLength  = ival(row, cidx["A653MsgLength"])
        except:
            if direction == 'input':
        ...
        if self.portType[0] == "Q":
            try:
                self.queueLength    = ival(row, cidx["PortQueueLength"])
            except:
        ...
        try:
            self.msgRate        = ival(row, cidx["TxInterval"])        
        except:
        ...
        try:
            self.msgValidityPeriod = ival(row, cidx["A653PortRefreshPeriod"])        
        except:
        ...
        try:
            if self.direction == 'input':
                self.portId         = ival(row, cidx["RxComPortID"])
            else:
                self.portId         = ival(row, cidx["TxComPortID"])                
        except:            
            raise Exception, "No PortID defined"

        try:
            self.portName         = sval(row, cidx["A653PortName"])
        except:            
...
class A429Label(object):
    ...
    def __init__(self, name, id, row, cidx, direction):
        ...
        try:
            ...
            raise Exception, "Bad value for A429 Label transmission rate"
        try:
            self.msgLabel       = ival(row, cidx["Label"], base=16)
        except:            
            raise Exception, "Bad value for Label Number"
        try:
            self.msgPhysPort    = sval(row, cidx["PhysPort"])
        except:            
            raise Exception, "Bad value for CAN Receive Port"
...
class InputSignal(object):
    def __init__(self, row, cidx, msgref, ddmap, paramxref, noparams=False, rowidx=0):
        ...
        self.rpName           = sval(row, cidx["RpName"])
        ...
        self.sigName          = sval(row, cidx["DpName"])
        self.sigDsOffset      = ival(row, cidx["BitOffsetWithinDS"])
        self.sigSize          = ival(row, cidx["ParameterSize"])
        self.sigType          = normalizeSigType(sval(row, cidx["ParameterType"]), self.sigSize)

        if self.sigType in ("BNR","UBNR","BCD"):
            self.sigLsbValue  = fval(row, cidx["LsbRes"])
        else:
            self.sigLsbValue  = 1
        ...
        sselstr     = sval(row, cidx["ValidityCriteria"])
        ...
        if ddmap != None:
            self.selectionSet   = sval(row, cidx["SelectionSet"])
        
            if self.paramDatatype == "FLOAT":
                self.paramDefault   = fval(row, cidx["DefaultVal"])
                self.paramMin       = fval(row, cidx["FunctionalMinRange"])
                self.paramMax       = fval(row, cidx["FunctionalMaxRange"])
            else:
                self.paramDefault   = ival(row, cidx["DefaultVal"])
                self.paramMin       = ival(row, cidx["FunctionalMinRange"])
                self.paramMax       = ival(row, cidx["FunctionalMaxRange"])
...
class MapReader(object):
    def __init__(self, fname, dd=None, msgonly=False, noparams=False, direction='input'):
        ...
        if direction == 'input':
            ...
            try:
                ...
                opA429LabelSheet  = workbook.sheet_by_name('OutputA429Labels')
            except Exception, msg:
                raise Exception, "File %s open failed!: %s" % (filename, str(msg))
    
            self.afdx  = self.readAfdxMessages(opAfdxMsgSheet, direction)
            self.can   = self.readCanMessages(opCanMsgSheet, direction)
            self.a429    = self.readA429Labels(opA429LabelSheet, direction)
    ...
    def readA429Labels(self, sheet, direction):
        cidx = getColumnIndex(sheet)
        messages    = dict()
        messageKeys = []
        msgidx      = 0
        for row_idx in range(1, sheet.nrows):
            ...
            msgname = lru + '.' + message
            try:
                messages[msgname] = A429Label(msgname, msgidx, row, cidx, direction)
            except Exception, msg:
                self.logerr("A429 %s labels" % direction, row_idx, str(msg) + ' - label skipped')
                continue
            messageKeys.append(msgname)
        return Bunch(messages=messages, messageKeys=messageKeys)
    ...
    def readInputSignals(self, ddmap, sheet, noparams):
        ...
        for row_idx in range(1, sheet.nrows):
            ...
            if message is None:
                ...
                if message is None:
                    message = self.a429.messages.get(msgName)
                    if message is None:
            ...
            if sig.paramOffset != -1 or noparams:
                if message.msgclass == 'AFDX':
                    ...
                    dskey = message.msgid
                    self.a429.datasets[dskey].append(sig)
